...
3. `docs/strategies.md` — the built-in strategy (VWAP ORB) and its actual rules; why the 3 daily-consuming strategies were retired
...
Newest strategy: **VWAP ORB Momentum (Long)** (`vwaporb`, preset #8) — a momentum opening-range-breakout confluence: on a Mon/Wed trending session, a liquid (≥30L/day) ₹500+ stock breaks above its 30-min OR high while holding a rising VWAP (slope 20–50 bps) on a non-negative gap day; held to 15:00. Developed via the offline diagnostic harness (`tools/vwap-diag*.cs`, non-lookahead) and **in-app validated**: 500-day run = **144 trades, +₹196/trade, 49% win, 16/24 months positive, +₹28,165 net**; consistent with the 250-day run (+₹184/trade). It replaced an earlier *VWAP Bounce* experiment that failed in-app. **Two hard-won lessons baked into `docs/strategy-optimization-playbook.md`:** (1) systematic VWAP *mean-reversion* (bounce/reclaim/fade) has no broad edge on NSE 5-min after cost — only this *momentum* confluence worked; (2) the offline harness over-states magnitude (it reported ~₹466/trade vs the real ~₹196) — **always reconcile in-app before trusting a number.** Regime-dependent (strong in trending quarters, weak in chop); a Nifty-VWAP regime filter is the natural next improvement. **VWAP ORB is now the sole built-in preset.** IN-SAMPLE — paper-test pending.

**Retired 2026-06-07 (`RemoveCorruptedPresets`):** Gap Fade (Long), Volume Confluence Breakout (Long), and EMA Gap-Down Reclaim (Long) were removed (classes + presets + run history). All three consumed daily candles and their backtests were inflated by a **daily-bar look-ahead** — a daily candle for IST trading day *D* was parsed via `.UtcDateTime` (no IST offset) and landed on *D−1*, so the orchestrator's `daily.Where(c => c.Timestamp.Date < day.Date)` admitted *today's own* bar as the "previous day" and the screeners read today's close as `previousDayClose`. Fixed in `DhanDataApiClient.GetDailyHistoricalAsync` (shift daily bars +5:30, daily path only; intraday stays UTC); the stale daily cache must be purged after the fix or the duplicate-dated bars re-poison runs. On clean data all three had no edge; Volume Confluence also relied on an unachievable stop-market fill **at OR.High** (~93% of its edge). VWAP ORB never used daily candles and was unaffected. See `docs/strategies.md`.
